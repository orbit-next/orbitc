...
106     NEXT i
        ...
        IF CONflag = 1 AND CONtarg = 12 THEN CONflag = 0.25
...
118         difX = cenX - Px(i, 3)
            ...
            angle = FIX(angle + 0.5) / rad / 160  '32
...
128         CIRCLE (300 + (Px(i, 3) - cenX) * mag / AU, 220 + (Py(i, 3) - cenY) * mag * 1 / AU), mag * P(i, 5) / AU, clr
            CIRCLE (300 + (Px(i, 3) - cenX - (P(i, 5) * 0.8 * SIN(vnSa))) * mag / AU, 220 + (Py(i, 3) - cenY - (P(i, 5) * 0.8 * COS(vnSa))) * mag * 1 / AU), mag * P(i, 5) * 0.2 / AU, clr
            CIRCLE (300 + (Px(i, 3) - cenX - (P(i, 5) * 1.2 * SIN(vnSa + 0.84))) * mag / AU, 220 + (Py(i, 3) - cenY - (P(i, 5) * 1.2 * COS(vnSa + 0.84))) * mag * 1 / AU), mag * P(i, 5) * 0.2 / AU, clr
            CIRCLE (300 + (Px(i, 3) - cenX - (P(i, 5) * 1.2 * SIN(vnSa - 0.84))) * mag / AU, 220 + (Py(i, 3) - cenY - (P(i, 5) * 1.2 * COS(vnSa - 0.84))) * mag * 1 / AU), mag * P(i, 5) * 0.2 / AU, clr
            RETURN
...
148         PSET (300 + (((Px(37, 3) + panel(0, 229)) - cenX) * mag / AU), 220 + (((Py(37, 3) + panel(1, 229)) - cenY) * mag / AU)), clr
            ...
            'Paint AYSE
158         Ax1 = Px(32, 3) + (500 * SIN(AYSEangle + 0.19 + pi))
            Ax2 = Px(32, 3) + (500 * SIN(AYSEangle - 0.19 + pi))
            Ay1 = Py(32, 3) + (500 * COS(AYSEangle + 0.19 + pi))
            Ay2 = Py(32, 3) + (500 * COS(AYSEangle - 0.19 + pi))
...
166     Rmin = 1E+26
        ...
        FOR j = 241 TO 265
            ...
            IF r >= Rcrit - 0.5 THEN 169
            eng = 0: explFLAG1 = 1
            Px(28, 3) = Px2 + ((Rcrit - 0.1) * SIN(Acon + 3.1415926#))
            Py(28, 3) = Py2 + ((Rcrit - 0.1) * COS(Acon + 3.1415926#))
...
193     IF ((Dcon - P(CONtarg, 5) - P(28, 5)) > -0.5) THEN GOTO 112
194     eng = 0
        ALTdel=0
        IF CONtarg=4 THEN ALTdel=MARSelev
        Px(28, 3) = Px(CONtarg, 3) + ((P(CONtarg, 5) + P(28, 5) - 0.1 + ALTdel) * SIN(Acon + 3.1415926#))
        Py(28, 3) = Py(CONtarg, 3) + ((P(CONtarg, 5) + P(28, 5) - 0.1 + ALTdel) * COS(Acon + 3.1415926#))
        explFLAG1 = 1
...
123     oldAcctarg = Acctarg
        ...
        IF LOS + INS = 0 AND Dtarg > 1E+9 THEN targDISP = 0
...
144     LOCATE 13, 2
        ...
        IF RADAR = 2 AND Dtarg < 1E+9 THEN zDISP$ = "###########.###": Dfactor = 1000
125     PRINT USING zDISP$; (Dtarg - P(targ, 5) - P(28, 5) + RcritL) / Dfactor;
        LOCATE 15, 9: PRINT USING "####.##"; Atargref * RAD;
129     IF Cdh > 0.0005 THEN COLOR 14
        LOCATE 7, 8: IF Cdh < 0.0005 THEN PRINT USING "#####.###"; Are; ELSE PRINT USING "#####.##"; Are; : PRINT "P";
        COLOR 15
...
113     'Timed back-up
        ...
        IF COLeventTRIG = 1 THEN ts = 0.125: TSindex=4
        OLDts = ts
...
103     z$ = INKEY$
        ...
        IF z$ = "+" AND mag < 130000000000# THEN mag = mag / 0.75: GOSUB 405
        IF z$ = "-" AND mag > 6.8E-11 THEN mag = mag * 0.75:  GOSUB 405
...
110     IF z$ = "e" THEN ENGsetFLAG = 1 - ENGsetFLAG
        IF z$ = CHR$(0) + "S" THEN eng = eng + 0.1: GOSUB 400
        IF z$ = CHR$(0) + "R" THEN eng = eng - 0.1: GOSUB 400
        ...
        IF z$ = CHR$(0) + "H" THEN vern = 0.1: vernA = 0
        IF z$ = CHR$(0) + "K" THEN vern = 0.1: vernA = 90
        IF z$ = CHR$(0) + "M" THEN vern = 0.1: vernA = -90
        IF z$ = CHR$(0) + "P" THEN vern = 0.1: vernA = 180
...
105     IF z$ = "s" THEN GOSUB 600
        ...
        IF TSindex = 6 and TIMER - tttt < 0.01 THEN 103
        GOTO 100
...
301     IF explosion > 1 THEN Ztel(2) = 0: Ztel(1) = 1
        ...
        IF vernP! < 0.01 THEN vernP! = 0
        IF ABS(HABrotMALF) * ABS(eng) > 0.0001 THEN Sflag = 1
        IF (NAVmalf and 3840)>0 THEN Sflag=1
        IF ABS(HABrotate + 0.5) < 0.5 * (1 - (vernCNT * 0.8)) THEN HABrotate = 0
        HABrotate = HABrotate + (HABrotateADJ% * (10 ^ (-1 * vernCNT)))
        HABrotateADJ% = HABrotateADJ% * vernCNT
        HABrotate = HABrotate + (eng * HABrotMALF * 0.0095 * SGN(Ztel(2)))
        HABrotate = HABrotate + (SGN(vernP!) * (((768 AND NAVmalf) / 256) * 0.015))
        HABrotate = HABrotate - (SGN(vernP!) * (((3072 AND NAVmalf) / 1024) * 0.015))
        IF ABS(HABrotate) > 99 THEN HABrotate = SGN(HABrotate) * 99
        IF ABS(HABrotateADJ%) > 10 THEN HABrotateADJ% = SGN(HABrotateADJ%) * 10

        COLOR 15
        Aoffset = ATN((P(targ, 5) * 1.01) / (Dtarg + 0.0001)): Atarg = Atarg - (Aoffset * OFFSET)
        difX = Vx(targ) - Vx(28)
        difY = Vy(targ) - Vy(28)
        GOSUB 5000
        Vvtangle = angle
...
303     IF (3840 AND NAVmalf) > 0 THEN vernP! = vernP! - 0.5: Sflag = 10
        ...
        IF vernP! < 0.01 AND Sflag <> 1 THEN Sflag = 10
        IF Sflag = 10 THEN Sflag = 1: LOCATE 25, 11: PRINT "MAN      ";
        IF Sflag = 1 THEN Sangle = Sangle + (HABrotate * 0.0086853 * ts): GOTO 302
        vernP! = vernP! - 0.01
        ...
        IF ABS(diffSangle) < 0.24 * ts THEN Sangle = dSangle: HABrotate = 0: GOTO 302
        Sangle = Sangle + (0.2 * ts * SGN(diffSangle))
...
319     Agrav = (THRUSTx - (Are * SIN(VvRangle))) ^ 2
        ...
        IF THRUSTy = 0 THEN IF THRUSTy < 0 THEN THRUSTangle = 0.5 * 3.1415926535# ELSE THRUSTangle = 1.5 * 3.1415926535# ELSE THRUSTangle = ATN(THRUSTx / THRUSTy)
...
405     CLS
        ...
        HPdisp = 0
400     IF mag < 0.1 THEN GOSUB 8000

        IF ts < 0.015625 THEN ts = .015626: TSindex=1
        IF ts > 60 THEN ts = 60: TSindex=17

        IF Dfuel > 2 THEN Dfuel = 0
        IF ufo2 = 1 THEN ts = 0.25: TSindex=5
        COLOR 8
...
818     k = 2
        ...
        IF Ztel(8) = 1 THEN ts = 0.125:TSindex=4
        'Ztel(9) = rshield
        ...
        IF (2 AND Ztel(10)) = 2 THEN Cdh = 0.0006 ELSE Cdh = .0002
...
3000    GOSUB 3005
        ...
        r# = SQR((difX ^ 2) + (difY ^ 2))
        term# = (L# / r#) - 1
        IF ABS(ecc) < 0.0000001# THEN ecc = SGN(ecc) * 0.0000001#
        term# = term# / ecc
...
3019        difX# = 300 + ((difX# - cenX) * mag / AU)
            ...
            FRAMEflag = 1
3002    NEXT i
        IF ecc < 1 AND lim2 = 179 THEN lim1 = 179: lim2 = 181: stp = 0.001: GOTO 3003
        IF ecc < 1 AND lim2 = 181 THEN lim1 = 181: lim2 = 359.9: stp = 0.1: GOTO 3003
        GOSUB 3020
        RETURN
...
5000    IF difY = 0 THEN IF difX < 0 THEN angle = 0.5 * 3.1415926535# ELSE angle = 1.5 * 3.1415926535# ELSE angle = ATN(difX / difY)
...
6001    'LOCATE 1, 35
        ...
        IF explCENTER = 28 OR explCENTER = 32 THEN ts = 0.25:TSindex=5
...
8100    time# = (year * 31536000#) + (day * 86400#) + (hr * 3600#) + (min * 60#) + sec
        ...
        IF TIMEsgn = -1 AND dtime# < 121 THEN ts = 0.125: TSindex=4
